...
class WeightedMCTS(MCTS):
    ...
    @staticmethod
    def new_node(
        policy: chex.Array, value: float, embedding: chex.ArrayTree, terminated: bool
    ) -> WeightedMCTSNode:
    ...
    def backpropagate(
        self, key: chex.PRNGKey, tree: MCTSTree, parent: int, value: float
    ) -> MCTSTree:
        ...
        def body_fn(state: BackpropState) -> Tuple[int, MCTSTree]:
            ...
            if self.q_temperature > 0:
                ...
                noise = jax.random.uniform(
                    key, shape=normalized_q_values.shape, maxval=self.tiebreak_noise
                )
