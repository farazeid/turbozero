...
def two_player_game(
    ...
    def step_step(state: TwoPlayerGameState, _) -> TwoPlayerGameState:
        ...
        state = jax.lax.cond(
            state.completed,
            lambda s: s,
            lambda s: jax.lax.cond(p1_first, step_p1, step_p2, s),
            state,
        )
        ...
        state = jax.lax.cond(
            state.completed,
            lambda s: s,
            lambda s: jax.lax.cond(p1_first, step_p2, step_p1, s),
            state,
        )
        # collect render frame
        frame2 = GameFrame(
    ...
    # play the game
    state, frames = jax.lax.scan(step_step, state, xs=jnp.arange(max_steps // 2))
    # reshape frames
