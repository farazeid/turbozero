...
def render_pgx_2p(
    frames, p_ids, title, frame_dir, p1_label="Black", p2_label="White", duration=900
):
    ...
    trained_agent_color = (
        p1_label if frames[0].env_state.current_player == p_ids[0] else p2_label
    )
    ...
    for i, frame in enumerate(frames):
        ...
        p1_text = ET.Element(
            "ns0:text",
            x=str(0.01 * original_width),
            y=str(original_height * 1.05),
            fill="white",
        ...
        new_area = ET.Element(
            "ns0:rect",
            fill="#1e1e1e",
            height=str(new_height - original_height),
            width=str(original_width),
            x="0",
            y=str(original_height),
        )
        ...
        cairosvg.svg2png(
            url=f"{frame_dir}/{num}.svg", write_to=f"{frame_dir}/{num}.png"
        )
    ...
    images[0].save(
        gif_path,
        save_all=True,
        append_images=images[1:] + ([images[-1]] * 2),
        duration=duration,
        loop=0,
    )
