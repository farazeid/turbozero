...
class Tree(Generic[NodeType]):
    ...
    def add_node(
        self, parent_index: int, edge_index: int, data: NodeType
    ) -> Tree[NodeType]:
    ...
    def get_subtree(self, subtree_index: int) -> Tree:
        ...
        def translate_pytree(x, null_value=self.NULL_VALUE):
            return jax.tree_map(lambda t: translate(t, null_value=null_value), x)
...
def init_tree(max_nodes: int, branching_factor: int, template_data: NodeType) -> Tree:
    ...
    return Tree(
        ...
        data=jax.tree_util.tree_map(
            lambda x: jnp.zeros((max_nodes, *x.shape), dtype=x.dtype), template_data
        ),
    )
